...
function init(canvas, opts = {}) {
  ...
  /* ---------- Lights ---------- */
  scene.add(new THREE.AmbientLight(0x8a897f, 0.55));

  const key = new THREE.DirectionalLight(0xe0ded7, 1.5);
  ...
  const glow = new THREE.PointLight(0xfbbb21, 60, 26, 1.8);
  ...
  const palette = [
    new THREE.Color(0x20201d),
    new THREE.Color(0x2b2b27),
    new THREE.Color(0x383733),
    new THREE.Color(0x4a4a45),
  ];
  const accent = new THREE.Color(0xfbbb21);
  ...
  const ground = new THREE.Mesh(
    ...
    new THREE.MeshLambertMaterial({ color: 0x141412 })
  );
  ...
  function renderFrame(t) {
    ...
    glow.intensity = 50 + Math.sin(t * 1.4) * 14;
